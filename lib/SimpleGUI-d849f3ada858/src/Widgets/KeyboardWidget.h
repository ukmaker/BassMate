...
class KeyboardWidget : public Widget {
 public:
  ...
  enum State { KEYS, TEXT, OK, CANCEL, USER };
  ...
  KeyboardWidget(GraphicsContext *context, char *buf, uint8_t buflen)
      : Widget(context), _buf(buf), _buflen(buflen - 1), _state(KEYS) {
    _x = _lastX = 0;
    _y = _lastY = 0;
    _c = ' ';
 ...
 protected:
  ...
  void _noteChanged() {
    if (_state != _lastState) _changed = true;
    if (_x != _lastX) _changed = true;
    if (_y != _lastY) _changed = true;
    if (_len != _lastLen) _changed = true;
  ...
  bool _isDirtyKey(uint8_t x, uint8_t y) {
    if (x == _lastX && y == _lastY) return true;

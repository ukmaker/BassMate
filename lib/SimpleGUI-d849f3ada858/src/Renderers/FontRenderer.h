...
class FontRenderer : public Print {

public:
    FontRenderer(Adafruit_GFX* gfx, int16_t w, int16_t h);
    ~FontRenderer() { }
    ...
    virtual void drawChar(int16_t x, int16_t y, unsigned char c, uint16_t color, uint16_t bg, uint8_t size_x, uint8_t size_y) = 0;
    ...
    virtual void setTextWindow(int16_t x, int16_t y, int16_t w, int16_t h) = 0;
    virtual void setVAlign(VAlign align) = 0;
    virtual void setHAlign(HAlign align) = 0;
    virtual void writeAligned(const char* string) = 0;
    virtual void removeTextWindow() = 0;
    virtual uint8_t getCharWidth(const char c) = 0;
